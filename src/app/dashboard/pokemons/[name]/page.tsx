import { PokemonDetailsTs, PokemonResponse } from "@/app/pokemons";
...
export async function generateStaticParams() {
    ...
    const limit = 898 //? cantidod total de pokemons en la api para el dia 2 de febrero del 2024
    const offset = 0
    // ~ obtengo todol los pokemons
    const pks: PokemonResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=${limit}&offset=${offset}`)
    .then(res => res.json())

    // -- 898 pokemon existen para el dia que escribi este codigo
    const staticPAgesIds = pks.results.map(pk => ({
        name: pk.name
    }))
    
    // -- 898 pokemon existen para el dia que escribi este codigo
    return staticPAgesIds.map( ({name}) => ({name}))
}

// * Para obtener lel listado de pokemos por medio de una peticion
const getPokemon = async (name: string): Promise<PokemonDetailsTs> => {
    try{
        //? en este caso funciona asi ya que la api se consume tambien permite buscar por name;
...
export default async function PokemonPage({ params }: props) {
    const pk = await getPokemon(params.name);
